...
1.6 + f002
	...
	* [nicklausw] Put unstable/highly unstable asm6f_opcode use behind directives,
...
1.5
	...
	Bugfix for ACC asm6f_opcode type
1.4
...
void dint32(label*,char**);
void dint24(label*,char**);
void dsint24(label*,char**);
void dsint32(label*,char**);
void dw(label*,char**);
void db(label*,char**);
void dl(label*,char**);
void dh(label*,char**);
...
enum optypes {ACC,IMM,IND,INDX,INDY,ZPX,ZPY,ABSX,ABSY,ZP,ABS,REL,IMP,ZPREL};
int opsize[]={0,1,2,1,1,1,1,2,2,1,2,1,0,3};
...
byte plx[]={0xfa,IMP,-1};
byte ply[]={0x7a,IMP,-1};
byte stz[]={0x74,ZPX,0x64,ZP,0x9c,ABS,0x9e,ABSX,-1};
...
struct {
	...
	void (*func)( label*, char** );
		...
		{"HEX",hex},
		{"INT32",dint32},{"DW32",dint32},{"DCW32",dint32},{"DC.W32",dint32},
		{"INT24",dint24},{"DW24",dint24},{"DCW24",dint24},{"DC.W24",dint24},
		{"INT16",dw},{"DW16",dw},{"DCW16",dw},{"DC.W16",dw},
		{"INT8",db},{"DW8",db},{"DCW8",db},{"DC.W8",db},
...
void initlabels(void) {
	...
	do {//opcodes first
		...
		p=newlabel();
		(*p).name=rsvdlist[i];
		(*p).value=(ptrdiff_t)asm6f_opcode;
		(*p).line=rsvdlist[i+1];
		(*p).type=RESERVED;
		i+=2;
	} while(rsvdlist[i]);
...
/*  static int noentry=0;
	...
	if(oldpass!=pass) {
		...
		if (c64_prg)
		{
			int realaddr=(int)addr-1;
			byte msb,lsb;
			msb = realaddr>>8;
			lsb = realaddr&0xff;
			fwrite(&lsb,1,1,outputfile);
			fwrite(&msb,1,1,outputfile);
			filepos+=2;
		}
...
void dint32(label *id, char **next) {
	int val;
	do {
		val=eval(next,WHOLEEXP);
		if(!errmsg) {
			if(val>2147483647 || val<-2147483647)
				...
				output_le(val,4,DATA);
		}
	} while(!errmsg && eatchar(next,','));
}

void dint24(label *id, char **next) {
	int val;
	do {
		val=eval(next,WHOLEEXP);
		if(!errmsg) {
...
void dsint32(label *id,char **next) {
...
void dsint24(label *id,char **next) {
	int count,val=defaultfiller;
	dependant=0;
	count=eval(next,WHOLEEXP);
	if(dependant || (count<0 && needanotherpass))//unknown count! don't do anything
...
void asm6f_opcode(label *id, char **next) {
	char *s,*s2;
	int type,val = 0;
	byte *op;
	...
	for(op=(byte*)(*id).line;*op!=0xff;op+=2) {//loop through all addressing modes for this instruction
		...
		my_strupr(s);
		s2=optail[type];
		while(*s2) {		//asm6f_opcode tail should match input:
			if(!eatchar(&s,*s2))
				break;
			s2++;
		}
...
#if 0
void c64prg(label *id, char **next) {
	c64_prg++;
}
